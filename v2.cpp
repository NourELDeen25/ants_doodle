...
{
    case 0:
        if((y>0)&&(world->get_pos(x,y-1) == nullptr))
        {
            world->set_pos(x,y-1, this);
            world->set_pos(x,y,nullptr);
            y--;
        }
    break;
    case 1:
        if((y<WorldSize-1)&&(world->get_pos(x,y+1) == nullptr))
            {
                world->set_pos(x,y+1, this);
                world->set_pos(x,y,nullptr);
                y++;
    ...
    case 2:
     if((x>0)&&(world->get_pos(x-1,y) == nullptr))
     {
        world->set_pos(x-1,y, this);
        world->set_pos(x,y,nullptr);
        x--;
     }
    break;
    ...
    case 3:
    if((x<WorldSize-1)&&(world->get_pos(x+1,y) == nullptr))
    {
            world->set_pos(x+1,y, this);
            world->set_pos(x,y,nullptr);
            x++;
...
void Doodlebug::move()
{
    ...
    if ((y>0)&&(world->get_pos(x,y-1) != nullptr)&&(world->get_pos(x,y-1)->getType()==ant))
    {
...
//        if((y>0)&&(world->get_pos(x,y-1) == NULL))
        ...
        //else if ((y<WorldSize-1)&&(world->get_pos(x,y+1)==NULL))
        else if ((y<WorldSize-1)&&(world->get_pos(x,y+1)==nullptr))    
        {
            Doodlebug* newDoodle= new Doodlebug(world, x, y+1);
        }
//        else if((x>0)&&(world->get_pos(x-1,y) == NULL))
        else if((x>0)&&(world->get_pos(x-1,y) == nullptr))
        {
            Doodlebug* newDoodle = new Doodlebug(world,x-1,y);
        }
        //else if ((x<WorldSize-1)&&(world->get_pos(x+1,y)==NULL))
        else if ((x<WorldSize-1)&&(world->get_pos(x+1,y)==nullptr)) 
        {
            Doodlebug* newDoodle= new Doodlebug(world, x+1, y);
        }
    }
